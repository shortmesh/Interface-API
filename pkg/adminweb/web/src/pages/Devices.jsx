...
import { apiCall, safeJsonParse, maskString, copyToClipboard } from '../utils/api'
...
export default function Devices() {
  ...
  const loadDevices = async () => {
    try {
      const tokenStatusResponse = await apiCall('/api/v1/admin/matrix-token-status')
  ...
  const handleSelectPlatform = async (platform) => {
    ...
    try {
      ...
      if (!qrCodeUrl || (qrCodeUrl.includes('token=') && qrCodeUrl.endsWith('token='))) {
      ...
      // Wait a bit before connecting websocket
      setTimeout(() => {
  ...
  const connectWebSocket = (qrCodeUrl) => {
    try {
      ...
      ws.onclose = () => {
        if (qrCodeData && connectionStatus !== 'error') {
    ...
    },
    {
      ...
      render: (deviceId, record, index) => (
        <Space size="small">
          ...
          >
            {revealedFields[`device-${index}`] ? deviceId : maskString(deviceId)}
          </Typography>
        ...
        <Space size="small">
          ...
          <Tooltip title="Delete">
            <IconButton
              color="error"
              size="small"
              onClick={() => handleDeleteDevice(device.device_id, device.platform)}
            >
              <Delete />
            </IconButton>
          </Tooltip>
  ...
  return (
    <Box>
      <Box sx={{ mb: 4, display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start' }}>
        ...
        <MuiButton variant="contained" startIcon={<Add />} onClick={handleOpenAddDevice}>
          Add Device
        </MuiButton>
      </Box>
        ...
        {platformStep ? (
          <Box sx={{ pt: 2 }}>
            ...
            <Grid container spacing={2} sx={{ mt: 2 }}>
              {[
                { platform: 'wa', name: 'WhatsApp', image: '/admin/whatsapp.png' },
                { platform: 'signal', name: 'Signal', image: '/admin/signal.png' },
                { platform: 'telegram', name: 'Telegram', image: '/admin/telegram.png' },
              ].map((item) => (
          ...
          <Box sx={{ pt: 2, textAlign: 'center' }}>
            ...
            {connectionStatus === 'error' && (
              <Alert severity="error">Connection failed. Please try again.</Alert>
            )}
          </Box>
        )}
      </Modal>
      ...
      >
        ...
        <Typography color="text.secondary" variant="caption" sx={{ mt: 2, display: 'block' }}>
          Your token will be stored for this session and will be cleared when you log out. <br />
          Don't have a token?{' '}
          <Link to="/tokens" style={{ color: '#4357AD', textDecoration: 'underline' }}>
            Create one here
          </Link>
        </Typography>
      </Modal>
      ...
      <Modal
        ...
        footer={[
          <Button key="cancel" type="text" onClick={() => setSendMessageDialogOpen(false)}>
            Cancel
          </Button>,
          <Button
            key="send"
